...
% This will merge a list of netcdf files within a specific time-space
% range.
% -------------------------------------------------------------------------
% INPUT:
%
...
if is_overlap_allowed
    ...
    %MV.all = [];
    MV_vals = cell(length(filepath_list), 1);

    for ii = 1:length( filepath_list )
        fprintf('Checking: %s\n', filepath_list{ii} );
        %tem = FUN_nc_varget( filepath_list{ii}, merge_dim_var_name );
        MV_vals{ii} = FUN_nc_varget( filepath_list{ii}, merge_dim_var_name );
...
cleanup_ncid0 = onCleanup(@() netcdf.close(ncid0));  % Auto-close on error or exit
...
cleanup_ncid1 = onCleanup(@() netcdf.close(ncid1));  % Auto-close on error or exit
...
for iv = 1:length(info0.Variables)
    ...
    if is_merge
        ...
        for ii = 1:length( filepath_list )
            ...
            cleanup_ncid2 = onCleanup(@() netcdf.close(ncid2));  % Auto-close on error
            ...
            % File will be closed automatically by cleanup_ncid2
...
% Files will be closed automatically by cleanup handlers
