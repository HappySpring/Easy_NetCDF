...
> **Notes: **  <mark>`FUN_nc_gen_presaved_netcdf_info` is replaced by `FUN_nc_gen_presaved_netcdf_info_v2` introduced in v1.50-beta.</mark> The new version saves the dimensional information in a new format ('v2'), dropping a lot of unnecessary information. The output .mat file is 10 times smaller than the previous one, with better performance, and related functions have been updated to support the new format.  
...
+ <mark>[new]</mark> Support resume from previous downloads with option 'resumable' set to true. (this is still an experimental feature)
+ Supports large datasets.
+ Retries automatically after interruptions.
+ Downloads data piece by piece, which is more stable for downloading large data via internet
...
| N_max_retry                | 10             |                                                              |
| var_exclude                | []             |                                                              |
| is_resumable               | false          | Make the downloads resumable by writing additional global attributes in the output netcdf file and a temporary file ending with `_rtmp.mat` |
